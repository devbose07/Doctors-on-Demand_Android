package com.globussoft.readydoctors.doctor.videochat;
...
public class CallActivity extends BaseLogginedUserActivity {
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		// System.loadLibrary("libjingle_peerconnection_so.so");
	...
	{
		call_direction_type = (Consts.CALL_DIRECTION_TYPE) extras.getSerializable(Consts.CALL_DIRECTION_TYPE_EXTRAS);
	...
	private void disableConversationFragmentButtons() {
		BaseConversationFragment fragment = (BaseConversationFragment) getFragmentManager()
	...
	private void initIncommingCallTask() {
		...
		showIncomingCallWindowTask = new Runnable() {
			@Override
			public void run() {
				IncomeCallFragment incomeCallFragment = (IncomeCallFragment) getFragmentManager()
						.findFragmentByTag(INCOME_CALL_FRAGMENT);
				if (incomeCallFragment == null) {
					BaseConversationFragment conversationFragment = (BaseConversationFragment) getFragmentManager()
							.findFragmentByTag(CONVERSATION_CALL_FRAGMENT);
				...
				// "Call was stopped by timer", Toast.LENGTH_LONG).show();
			}
		};
	}
	...
	public void rejectCurrentSession() {
		if (SessionManager.getCurrentSession() != null) {
			SessionManager.getCurrentSession().rejectCall(
	...
	public void hangUpCurrentSession() {
		if (SessionManager.getCurrentSession() != null) {
			SessionManager.getCurrentSession().hangUp(
					new HashMap<String, String>());
		}
		finish();
	}
	...
	private void startIncomeCallTimer() {
		showIncomingCallWindowTaskHandler.postAtTime(
				...
				SystemClock.uptimeMillis()
						+ TimeUnit.SECONDS.toMillis(QBRTCConfig
	...
	private void stopIncomeCallTimer() {
		...
		showIncomingCallWindowTaskHandler
				.removeCallbacks(showIncomingCallWindowTask);
	}
		...
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				if (hangUpReason != null
						&& hangUpReason.equals(Consts.WIFI_DISABLED)) {
					Intent returnIntent = new Intent();
					setResult(Consts.CALL_ACTIVITY_CLOSE_WIFI_DISABLED,
							returnIntent);
					finish();
				}
			}
	...
	// private void setStateTitle(final Integer userID, final int stringID,
	...
	// opponentItemView.findViewById(R.id.connectionStatus);
	// connectionStatus.setText(getString(stringID));
	//
	// ProgressBar connectionStatusPB = (ProgressBar)
	// opponentItemView.findViewById(R.id.connectionStatusPB);
	...
	public void addConversationFragment(List<Integer> opponents,
		...
		if (SessionManager.getCurrentSession() == null
				&& callDirectionType == Consts.CALL_DIRECTION_TYPE.OUTGOING) {
			//startOutBeep();
			...
			try {
				...
				System.out.println("opponents " + opponents.toString()
						+ "qbConferenceType   " + qbConferenceType);

				QBRTCSession newSessionWithOpponents = QBRTCClient
						.getInstance().createNewSessionWithOpponents(opponents,
								qbConferenceType);
				SessionManager.setCurrentSession(newSessionWithOpponents);
				Log.d(TAG, "addConversationFragmentStartCall. Set session "
		...
		BaseConversationFragment fragment = (QBRTCTypes.QBConferenceType.QB_CONFERENCE_TYPE_VIDEO.equals(qbConferenceType)) ? new VideoConversationFragment()
				: new AudioConversationFragment();

		Bundle bundle = new Bundle();
		bundle.putInt(Consts.CALL_DIRECTION_TYPE_EXTRAS,
				callDirectionType.ordinal());
		fragment.setArguments(bundle);

		getFragmentManager()
				.beginTransaction()
				.replace(R.id.fragment_container, fragment,
						CONVERSATION_CALL_FRAGMENT).commit();
	}
		...
		callBroadcastReceiver = new BroadcastReceiver() {
			...
			public void onReceive(Context context, Intent intent) {
				...
				if (intent.getAction().equals(Consts.CALL_RESULT)) {
					...
					int callTask = intent.getIntExtra(Consts.CALL_ACTION_VALUE,
							0);
					final Integer userID = intent
							.getIntExtra(Consts.USER_ID, 0);
